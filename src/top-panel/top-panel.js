import React from 'react';

import "./top-panel.css"

const TopPanel = ({ toggleIs3D }) => {
    return (
        <div className="pos-f-t">
            <nav className="navbar " >
                <h4 className="text col-md-3" >MC Skin Editor</h4>
                <div className="container col-md-6">

                    <div className="btn-group" role="group" >
                        <button id="2d" className="btn " type="button" onClick={toggleIs3D}>2d</button>
                        <button id="3d" className="btn " type="button" onClick={toggleIs3D}>3d</button>
                    </div>
                    <div className="btn-group" role="group">
                        <button className="btn  " type="button" >undo</button>
                        <button className="btn  " type="button" >redo</button>
                    </div>
                    {/*<div className="btn-group" role="group">
                        <button className="btn  " type="button" >-</button>
                        <input className="input " placeholder="100%" size="auto"></input>
                        <button className="btn  " type="button" >+</button>
                    </div>
                    */}
                </div>
                <div className="btn-group d-flex col-md-3" role="group" >
                    <button className="btn btn-primary" type="button" >AIPhoto</button>
                    <button className="btn btn-primary" type="button" >Export</button>
                </div>
            </nav>
        </div>
    );
};

export default TopPanel;